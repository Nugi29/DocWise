    ...
    <div className='m-5'>
      <div className='flex flex-wrap gap-3'>
        ...
        <div className='flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-green-100 cursor-pointer hover:scale-105 transition-all'>
          ...
          <div>
            <p className='text-xl font-semibold text-gray-600'>{currency}{dashData.earnings}.00</p>
      ...
      <div className='bg-white'>
        ...
        <div className='pt-4 border border-t-0 '>
          {dashData.latestAppointments.map((item, index) => (
            <div className='flex items-center px-6 py-3 gap-3 hover:bg-green-50' key={index}>
              <img className='rounded-full w-10' src={item.userData.image} alt="" />
              <div className='flex-1 text-sm'>
                <p className='text-gray-800 font-medium'>{item.userData.name}</p>
                <p className='text-gray-600'>{slotDateFormatter(item.slotDate)}</p>
              </div>
              {
                item.cancelled
