...
const RelatedDoctors = ({ speciality, docId }) => {
    ...
    useEffect(() => {
        if (doctors.length > 0 && speciality) {
            const docatorsData = doctors.filter(doc => doc.speciality === speciality && doc._id !== docId);
            setRelDoc(docatorsData);
        }
    ...
    return (
        <div>
            <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
                ...
                <button onClick={() => { navigate('/doctors'); scrollTo(0, 0); }} className='bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10 cursor-pointer'>More Doctors</button>
            </div>
        </div>
    )
}
